...
- The viewer now sends the documented fixed-length 8-channel RC packet used by the vehicle node. The local viewer input model still starts from `throttle + steering + head pose`, then mixes those semantics into RC channels before transmission.
